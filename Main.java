...
class Item {
  ...
  public Item(String name, int price) { //Passing the price and name
    this.name = name;
    this.price = price;
  }

  public String toString() {  //This method is for price
...
public class Main {//Main class 
  public static void main(String[] args) throws Exception {
    FileInputStream fis=new FileInputStream("input.txt");       // Passing the input file which we have created
    ...
    FileWriter fw = new FileWriter("output.txt"); //Display the output
    fw.write("The goodies selected for distribution are:\n\n");
    for(int i=min_index;i<min_index + number_of_employees; i++) {
      fw.write(goodies_items.get(i).toString() + "\n");
    }

    fw.write("\nAnd the difference between the chosen goodie with highest price and the lowest price is " + min_diff); //Displaying the diffence between the two goodie
	  fw.close();
  }
}